...
class ShoppingViewModelTest {
    ...
    @get:Rule
    var instantTaskExecutorRule = InstantTaskExecutorRule() // make sure every test will run on one thread one after another
    ...
    fun `insert shopping item with empty field, returns error`() {
        ...
        // first time we call this function, will just return the content
        assertThat(value.getContentIfNotHandled()?.status).isEqualTo(Status.ERROR)
    }

    @Test
    fun `insert shopping item too long name, returns error`() {
    ...
    @Test
    fun `insert shopping item too long price, returns error`() {
        val string = buildString {
